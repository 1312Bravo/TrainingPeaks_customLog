...
from helpFunctions import replace_nan_with_empty_string
    ...
    if not activity_stats:
        ...
        for i in reversed(range(nr_activities)):
            ...
            singleActivity_activityScores = {
                ...
                "Average pace [min/km] or speed [km/h]": round(60 / (activity_averageSpeed * 3.6), 1) if activity_parenttypeId == 1 else round(activity_averageSpeed * 3.6, 1) if activity_parenttypeId == 2 else np.nan,
                "Gradient adjusted pace [min/km]": round(60 / (activity_avgGradeAdjustedSpeed * 3.6), 1) if activity_parenttypeId == 1 else np.nan,
